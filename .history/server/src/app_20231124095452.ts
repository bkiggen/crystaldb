...
class MessageClass {
  ...
  // Get latest message
  static getLatestMessage(): MessageT {
    return new MessageClass("1", fakeDatabase[Object.keys(fakeDatabase)[0]]);
  }
}
...
const root = {
  getMessage: ({ id }: { id: string }): MessageT => {
    if (!fakeDatabase[id]) {
      throw new Error("no message exists with id " + id);
    }
    return new MessageClass(id, fakeDatabase[id]);
  },
  getLatestMessage: (): MessageT => {
    return MessageClass.getLatestMessage();
  },
  createMessage: ({ input }: { input: MessageInput }): MessageT => {
